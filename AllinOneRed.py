...
def copy_files(A, B, comp):
    dir_all_Individuals='./all_Individuals/'
    dir_all_poscars='./all_poscars/'
    shutil.copyfile(os.path.join(os.path.dirname(__file__),f'A{comp}B/OUTPUTFILES/{A+B}/{A+B}_gatheredPOSCARS'), dir_all_poscars+f'{A+B}_gatheredPOSCARS')
    shutil.copyfile(os.path.join(os.path.dirname(__file__),f'A{comp}B/OUTPUTFILES/{A+B}/{A+B}_Individuals'), dir_all_Individuals+f'{A+B}_Individuals')
    return
...
def find_enthalpy_relaxed(df, gen_couple, rel_couple, sg):
    try:
        ...
        with open('log.txt','a') as fstdout:
                fstdout.write(f"Si è verificato un errore: {e}")
        return None
    return enthalpy
...
class TemplateSet:
    def __init__(self, test_elements, file = None,   comp = 1):
        ...
        self.df = pd.read_csv(os.path.join(os.path.dirname(__file__),f'A{comp}B/relaxation/RELAX_DATA'), sep=",", index_col=0, na_filter = False)
...
class PairSet:
    def __init__(self, template_set, test_elements, relaxed_pairs = None, comp=1 ) -> None:
        ...
        self.df = pd.read_csv(os.path.join(os.path.dirname(__file__),f'A{comp}B/relaxation/RELAX_DATA'), sep=",", index_col=0, na_filter = False)
        self.one_el = pd.read_csv(os.path.join(os.path.dirname(__file__),f'A{comp}B/relaxation/OneElementEnt.txt'), sep=',', header=None, na_filter=False)
        self.gs_df = pd.read_csv(os.path.join(os.path.dirname(__file__),f'A{comp}B/relaxation/GroundStates.txt'), sep=",", na_filter = False)
        self.couples_info = [(['Na', 'B'], 191, ['1a', '2d']), (['Na', 'Mg'], 191, ['1a', '2d']), (['Na', 'Be'], 191, ['1b', '2c']), (['Na', 'O'], 139, ['2a', '4e']), (['Na', 'O'], 166, ['3a', '6c']), (['Na', 'C'], 191, ['1b', '2c']), (['B', 'Mg'], 191, ['1a', '2d']), (['B', 'N'], 119, ['2a', '2c', '4e', '4f']), (['B', 'Be'], 216, ['4a', '4b', '4d']), (['Mg', 'Na'], 164, ['1b', '2d']), (['Mg', 'Be'], 140, ['4a', '8h']), (['Mg', 'O'], 129, ['2c', '2c', '2c']), (['Mg', 'Li'], 139, ['2a', '4e']), (['N', 'Mg'], 194, ['2a', '4f']), (['N', 'Mg'], 187, ['1a', '2i']), (['N', 'Mg'], 123, ['1d', '2g']), (['N', 'O'], 160, ['3a', '3a', '3a']), (['N', 'Li'], 194, ['2a', '4f']), (['N', 'Li'], 166, ['3a', '6c']), (['N', 'Li'], 187, ['1f', '2g']), (['Be', 'Na'], 164, ['1b', '2d']), (['Be', 'Mg'], 227, ['16d', '8a']), (['Be', 'N'], 166, ['3a', '3b', '6c', '6c']), (['Be', 'N'], 187, ['1a', '1e', '1f']), (['Be', 'O'], 156, ['1a', '1a', '1c']), (['Be', 'Li'], 164, ['2c', '2d', '2d']), (['Be', 'Li'], 164, ['1b', '2d']), (['Be', 'C'], 225, ['4a', '8c']), (['O', 'Na'], 123, ['1b', '2h']), (['O', 'Li'], 123, ['1a', '2h']), (['O', 'Li'], 127, ['2b', '4g']), (['O', 'C'], 136, ['2a', '4g']), (['O', 'C'], 166, ['3a', '6c']), (['Li', 'Mg'], 139, ['2a', '4e']), (['Li', 'Be'], 164, ['2c', '2d', '2d']), (['Li', 'Be'], 191, ['1b', '2c']), (['Li', 'O'], 225, ['4b', '8c']), (['Li', 'C'], 166, ['6c', '6c', '6c']), (['C', 'Na'], 191, ['1a', '2d']), (['C', 'B'], 141, ['4a', '8e']), (['C', 'B'], 164, ['2c', '2d', '2d']), (['C', 'Be'], 156, ['1a', '1a', '1c']), (['C', 'Li'], 191, ['1b', '2c']), (['C', 'Li'], 166, ['3b', '6c']), (['C', 'Li'], 166, ['6c', '6c', '6c'])]
...
def ent_single_elemet(file):
    df = pd.read_csv(file, sep=',', header=None, na_filter=False)
    return df          
...
def graph_difference_std (dif_mean, dif_std, n_temp, c_value, dir_temp):
...
for lev_red_var in range(0.5,0.9,0.1):
    for n_pairs_var in range(20,100,20):
        hyperparameters['lev_red'] = lev_red_var
        hyperparameters['n_pairs'] = n_pairs_var
...
# Errore dei vari set con fissato numero di template estratti, su ogni possible coppia
        ...
        for i in tqdm(range(hyperparameters['ntemp_start'],hyperparameters['ntemp_end'], 2)):
            ...
            graph_difference_std(dif_mean[i-hyperparameters['ntemp_start']] , dif_std[i-hyperparameters['ntemp_start']] , i, (i-hyperparameters['ntemp_start'])/ntemp_studied, dir_temp )
