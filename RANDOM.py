...
import template_csp.managetemp as mte
...
distances = [dist2, dist3]
lev_reds = [ 0.14, 0.11]
n_pairs = [20, 20]

for dist_function in distances:

    hyperparameters['lev_red'] = lev_reds[distances.index(dist_function)]
    hyperparameters['n_pairs'] = n_pairs[distances.index(dist_function)]

    n_possible_couples = 105

    dir_temp = f'./DIFFERENT_DISTANCES_RED/{dist_function.__name__}/'
    ...
    for i in tqdm(range(hyperparameters['ntemp_start'],hyperparameters['ntemp_end'], 1)):
        ...
        for k in range(hyperparameters['n_sets']):
            ...
            template_set =  mte.TemplateSet(test_elements, f'RANDOM/FULL50sets/{hyperparameters["n_template"]}/TemplateSet_{k}') # generate_one_templateset(hyperparameters, test_elements, dist_function)

            reduced_set = generate_one_pairset(template_set, hyperparameters, test_elements, dist_function)
